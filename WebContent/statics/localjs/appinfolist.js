...
$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:path+"/devappinfo/queryCategoryListByParentId",//请求的url
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href=path+"devappVersion/todevAppVersionAdd/"+ obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href= path + "devappVersion/todevAppVersionModify?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href=path+"devappinfo/devtomodifyAppInfo";
	}else{
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href=path+"devappinfo/devgetAppInfo/"+ obj.attr("appinfoid");
});
...
$(".deleteApp").on("click",function(){
	...
	if(confirm("你确定要删除APP应用【"+obj.attr("appsoftwarename")+"】及其所有的版本吗？")){
		$.ajax({
			type:"GET",
			url:path+"/devappinfo/devdeleteApp",
